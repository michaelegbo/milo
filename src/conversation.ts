...
export function createConversation(options: {
  ...
  container.innerHTML = `
    ...
    <div class="conversation-controls"><button id="conversation-start" class="primary-button" disabled aria-label="Start conversation"><svg class="icon mic-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" aria-hidden="true"><rect x="9" y="2" width="6" height="12" rx="3"></rect><path d="M5 10v2a7 7 0 0 0 14 0v-2M12 19v3m-4 0h8"></path></svg><span id="conversation-start-label">Start conversation</span></button><button id="conversation-end" class="stop-button" disabled aria-label="End conversation" title="End conversation">■</button></div>
  ...
  function render() {
    ...
    const startLabel = state === 'listening' ? 'Send voice message' : ['transcribing', 'thinking', 'voicing'].includes(state) ? `${labels[state]}…` : micMuted ? 'Unmute & talk' : state === 'speaking' ? 'Interrupt & talk' : 'Start conversation';
    el('conversation-start-label').textContent = startLabel; start.setAttribute('aria-label', startLabel);
