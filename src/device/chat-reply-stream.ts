const CHAT_MAX_REPLY_LENGTH = 600;
...
export function createReplyStream(onChunk: (text: string) => void, onLimit: () => void = () => {}) {
  let pending = '';
  let text = '';
  let sentences = 0;
  let closed = false;
  ...
  function finish() {
    ...
    pending = '';
    if (!text) throw new Error('Milo could not form a reply. Please try a shorter question.');
    return text;
  }
  return { push, finish };
}
