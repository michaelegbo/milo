...
> This guide describes the optional local Node application and its earlier Windows verification. Hosted Milo uses browser workers instead; see the [main README](../README.md) for its architecture, privacy and current support.
...
This applies to GPU detection and switching at runtime. Initial `npm install` still installs the project's Node dependencies and can run their installation scripts, including native runtime package setup. These installation requirements apply to the local Node application. The separate hosted browser build requires no Node installation.
...
| **iOS / Android browsers** | The responsive layout is browser UI only. Milo does not run its Node/native inference backend or models directly in a mobile browser. This local Node launch does not run inference in the mobile browser; see the separate hosted device build in the root README. |
...
Milo's original code and assets are **source available under the PolyForm Noncommercial License 1.0.0**. Personal and other permitted noncommercial use, modification, and sharing are allowed under the [full license](../LICENSE). Commercial use is not granted by this license; contact the maintainer for separate permission. Because commercial use is restricted, Milo is not presented as OSI open source.

Third-party dependencies, fonts, models, and their assets retain their own licenses; the Milo license does not replace those terms.
